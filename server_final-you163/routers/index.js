...
router.post('/login_email', function (req, res) {
  ...
  console.log('/login_email', name, pwd, req.session)
  ...
  UserModel.findOne({name})
    .then((user) => {
      if (user) {
        if (user.pwd !== pwd) {
          ...
          res.send({code: 0, data: {_id: user._id, email: user.name, phone: user.phone}})
        }
        return new Promise(() => {
    ...
    .then((user) => {
      req.session.userid = user._id
      const data = {_id: user._id, email: user.name}
      // 3.2. 返回数据(新的user)
      res.send({code: 0, data})
    })
    .catch(error => {
      console.error('/login_email', error)
    })
})
